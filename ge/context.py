...
def prepare_issue(repo, number, *, output_dir='.ge', download_media_flag=True):
    ...
    context = {
        ...
        'media': {
            ...
            'all_visual_files': media_result['all_visual_files'] if media_result else [],
        },
...
def prepare_pr(repo, number, *, output_dir='.ge', download_media_flag=True,
               include_diff=True):
...
def render_pr_context(ctx):
    ...
    a(f"**Repo:** {ctx['repo']}  ")
    a(f"**URL:** {ctx['url']}  ")
    a(f"**State:** {ctx['state']} {'(MERGED)' if ctx.get('merged') else ''} {'(DRAFT)' if ctx.get('draft') else ''}  ")
    a(f"**Author:** @{ctx['author']}  ")
    a(f"**Branch:** `{ctx.get('head_branch', '?')}` → `{ctx.get('base_branch', '?')}`  ")
    a(f"**Created:** {ctx['created_at']}  ")
    a(f"**Labels:** {', '.join(ctx['labels']) or 'none'}")
    a(f"")
    ...
    if analysis.get('signals'):
        ...
        if analysis.get('linked_issues'):
            a(f"**Linked issues:** {', '.join(f'#{n}' for n in analysis['linked_issues'])}")
            a(f"")
