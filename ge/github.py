...
def get_comments(repo, number):
    ...
    owner, name = parse_repo_spec(repo)
    return gh_api(
        f'repos/{owner}/{name}/issues/{number}/comments', paginate=True,
    )
...
def get_review_comments(repo, number):
    ...
    owner, name = parse_repo_spec(repo)
    return gh_api(
        f'repos/{owner}/{name}/pulls/{number}/comments', paginate=True,
    )
...
def get_reviews(repo, number):
    ...
    owner, name = parse_repo_spec(repo)
    return gh_api(
        f'repos/{owner}/{name}/pulls/{number}/reviews', paginate=True,
    )
...
def get_pr_diff(repo, number):
    ...
    result = subprocess.run(
        ['gh', 'api', f'repos/{owner}/{name}/pulls/{number}',
...
def get_pr_files(repo, number):
    ...
    owner, name = parse_repo_spec(repo)
    return gh_api(
        f'repos/{owner}/{name}/pulls/{number}/files', paginate=True,
    )
